...
* openSUSE and derivatives::
    ...
    sudo zypper install bc binutils bison bzip2 dwarves flex gcc git \
      kernel-install-tools libelf-devel libzstd-devel make modutils \
      ncurses-devel  openssl openssl-devel perl-base qt6-base-devel rpm-build \
      xz-devel zlib-devel

To keep things simple, these lists includes some packages you in the end might
not require -- for example, the development libraries for ncurses and Qt6 will
only be needed in case you later want to adjust the build configuration with
make targets like 'menuconfig' or 'xconfig'.

[:ref:`back to step-by-step guide <buildrequires_bissbs>`]

.. _sources_bisref:

Downloading the sources using Git
---------------------------------
 ...
 * If you have an unreliable internet connection, you might want to
   :ref:`use a 'Git bundle'<sources_bundle_bisref>`.
